...
function Form(props) {
    ...
    const [errormessage, setErrorMessage] = useState ("* Revisa que no haya campos en rojo y que se hayan subido las fotos correctamente (max. 20KB).");
    ...
    const checkValidInput = () => {
      ...
      if (props.object.photo === "") {
        errorClone.photo ='Falta subir la foto de la autora (max. 20KB).';
        respuesta = true; 
      }else{
        errorClone.photo = "";
      }
      if (props.object.image === "") {
        errorClone.image='Falta subir la foto del proyecto (max. 20KB).';
        respuesta = true; 
      }else{
        errorClone.image = "";
      }
